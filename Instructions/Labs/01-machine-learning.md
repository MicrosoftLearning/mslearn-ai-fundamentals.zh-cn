...
1. 在 Azure 机器学习工作区资源中，选择“**启动工作室**”（或打开新的浏览器选项卡并导航到[https://ml.azure.com](https://ml.azure.com?azure-portal=true)，然后使用 Microsoft 帐户登录到 Azure 机器学习工作室）。 关闭显示的任何消息。
...
1. 通过以下设置创建新自动化 ML 作业，并根据需要使用“下一步”来逐步完成用户界面：
   ...
   任务类型与数据：
    ...
    - 选择数据集：使用以下设置创建新数据集：
        ...
        - **MLtable 选择**：
            - **上传文件夹**：*下载并解压包含需要上传的两个文件的文件夹* `https://aka.ms/bike-rentals`
