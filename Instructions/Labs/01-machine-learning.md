...
>**重要说明**目前，可以在 Azure 机器学习工作室中创作模型并将其部署为 Web 服务，但不能在工作室中对其进行测试。 因此，在清理之前，除了最后一部分的步骤外，其余步骤均可完成。 一旦发生变化，我们将会进行更新。
...
1. 通过以下设置创建新自动化 ML 作业，并根据需要使用“下一步”来逐步完成用户界面：
   ...
   任务类型与数据：
    ...
    - 限制：展开此节
        ...
        - 最大并发试用数：3
        - 最大节点数：3
        - **指标分数阈值**：0.085（如果模型的标准均方根误差指标分数为 0.085 或更低，作业会结束。）**
        - 超时：15
        - 迭代超时****：15
        - 启用提前终止：已选择
    - 验证和测试：
...
>**重要说明**Azure 机器学习工作室目前不支持使用部署测试所需的数据集创建类型。 在解决方案推出后，我们将及时通知你。 
